...
class WhitelistChecker(QMainWindow):
    ...
    def check_finished(self):
        ...
        # Формируем минимальные подсети
        self.calculate_minimal_subnets()

        self.status_label.setText(
            f"Проверка завершена. Доступно {len(self.alive_addresses)} из {len(self.addresses)} адресов"
        )

    def calculate_minimal_subnets(self):
        ...
        for address in self.alive_addresses:
            if "/" in address:
                ...
                try:
                    ...
                    # Это домен, преобразуем в IP-адреса
                    try:
                        # Используем nslookup для получения IP-адресов домена
                        result = subprocess.run(
                            ["nslookup", address],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
        ...
        if ip_addresses:
            try:
                # Используем netaddr для объединения IP в минимальные подсети
                ip_set = IPSet(ip_addresses)
                ip_networks = list(ip_set.iter_cidrs())
                minimal_subnets = cidr_merge(ip_networks)
                ...
                for subnet in minimal_subnets:
                    self.subnet_results.append(str(subnet))
            except Exception as e:
                self.subnet_results.append(f"Ошибка при формировании подсетей: {e}")
